...
class ControladorAdotantes:
    ...
    def listar_adotantes(self):
        ...
        if tam_lista_adotantes > 0:
            adotantes_list = [[adotante.cpf, adotante.nome, adotante.data_nascimento.strftime("%d/%m/%Y"), adotante.endereco, "S" if adotante.tem_outros_animais else "N", adotante.tipo_habitacao] for adotante in self.__adotantes]
            self.__tela_adotante.mostrar_adotantes(
                    adotantes_list
                )    
        else:
            self.__tela_adotante.mostra_mensagem(
